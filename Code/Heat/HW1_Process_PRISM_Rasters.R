...
# will go.
setwd("C:/Users/ulric/OneDrive - University of North Carolina at Chapel Hill/DIS/Heatwave_Mobility/Data/Temp/Raw")
...
prism_set_dl_dir(file.path(getwd(), "Data/Temp/Raw/tmean"))
...
dir.create("Data/Temp/Raw/tdmean", showWarnings = FALSE)
...
for (temp_var in temp_vars) {
  ...
  dat.files <- list.files(
    path = paste0("Data/Temp/Raw/", temp_var),
    recursive = TRUE,
    pattern = "_bil.bil$",
    full.names = TRUE
  )
  
  # Extract YYYYMMDD from filenames, convert to Date
  file_dates <- str_extract(basename(dat.files), "\\d{8}") %>%
    as.Date(format = "%Y%m%d")
  
  # Order files by date
  ord <- order(file_dates)
  dat.files <- dat.files[ord]
  tempo <- file_dates[ord]
  
  # Now stack in correct order
  mystack <- stack(dat.files)
  r <- setNames(mystack, tempo)
  
  # Reproject shapefile to match raster CRS
  CBG2010Shape_proj <- st_transform(CBG2010Shape, st_crs(r))
  
  # Crop raster to shapefile extent
  r.crop <- crop(r, extent(CBG2010Shape_proj))
  
  # Extract mean values by polygon
  r.mean <- raster::extract(r.crop, CBG2010Shape_proj, method = "simple", fun = mean, sp = TRUE)
  ...
  write_parquet(data, paste0("Data/Temp/Processed/CBG_2010/", tolower(temp_var), "_2012_2024_CBG2010.parquet")) 
...
data$heatindex <- heat.index(t = data$tmax, dp = data$tdmean, temperature.metric = "celsius", output.metric = "celsius")
