...
Temperature <- left_join(open_dataset("Data/NC_Tract_PRISM_2012_2024_Daily.parquet") %>% # Create a df of the temeprature data and rename the columns
                           mutate(month = month(Date)) %>%
                           collect(),
                         open_dataset("Data/NC_Tract_PRISM_2012_2024_Daily.parquet") %>% # Create a df for extreme percentile of temperatures
                           mutate(month = month(Date)) %>%
                           collect() %>%
...
EHF_pipeline_chunked <- function(dataset, chunk_size = 100) {
  # Get unique Tract values and split into chunks
  tract_chunks <- split(unique(dataset$Tract), 
                        ceiling(seq_along(unique(dataset$Tract)) / chunk_size))
  
  results <- list()
  
  # Process each chunk separately to avoid memory overload
  for (i in seq_along(tract_chunks)) {
    message(paste("Processing chunk", i, "of", length(tract_chunks)))
    
    subset_data <- dataset %>% filter(Tract %in% tract_chunks[[i]])
    
    chunk_result <- subset_data %>%
      dplyr::select(Tract, Date, TAVG, H95) %>%
      nest(data = c(-Tract)) %>%
...
# Run function in chunks of 100 tracts at a time
...
Temperature <- left_join(Temperature, Temperature_1 %>% dplyr::select(-TAVG, -H95), by = c("Tract", "Date")) 

write_parquet(Temperature, "Data/NC_EHF_Heatwave_Tract_2012_2024.parquet")
...
Heat_Calc <- Heatwave %>%
  ...
  dplyr::select(Date, Tract, TAVG, TMIN, TMAX, EHF, EHI_sig, EHI_accl, 
...
write_parquet(Heat_Calc, "Data/EHF_Heatwave_Metrics_NC_Tract_2012_2024.parquet")
